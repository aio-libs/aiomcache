...
async def test_pool_acquire_release(mcache_params, loop):
    pool = MemcachePool(minsize=1, maxsize=5, loop=loop, **mcache_params)
...
@pytest.mark.run_loop
async def test_pool_acquire_release2(mcache_params, loop):
    pool = MemcachePool(minsize=1, maxsize=5, loop=loop, **mcache_params)
...
@pytest.mark.run_loop
async def test_pool_clear(mcache_params, loop):
    pool = MemcachePool(minsize=1, maxsize=5, loop=loop, **mcache_params)
    conn = await pool.acquire()
    pool.release(conn)
    assert pool.size() == 1
...
async def test_acquire_limit_maxsize(mcache_params,
    ...
    await asyncio.gather(*([acquire_wait_release()] * 50), loop=loop)
    assert pool.size() == 1
...
async def test_acquire_task_cancellation(
    ...
    class Client:
        ...
        async def acquire_wait_release(self, conn):
            ...
            await asyncio.sleep(random.uniform(0.01, 0.02), loop=loop)
            return "foo"
...
@pytest.mark.run_loop
async def test_maxsize_greater_than_minsize(mcache_params, loop):
...
@pytest.mark.run_loop
async def test_0_minsize(mcache_params, loop):
...
@pytest.mark.run_loop
async def test_bad_connection(mcache_params, loop):
    pool = MemcachePool(minsize=5, maxsize=1, loop=loop, **mcache_params)
